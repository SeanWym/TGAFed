...
class ClientModule:
    ...
    def train_one_round(self, client_id, curr_round, weights=None, sigma=None, psi=None, helpers=None):
        ...
        #     self.train.get_c2s(), self.train.get_s2c(), self.prob_max_mu_t, self.prob_max_var_t, self.label_max_mu_t, self.label_max_var_t,self.sum_mask)
        return (self.get_weights(), self.get_train_size(), self.state['client_id'], 
    ...
    def set_prob_max_mu_t(self, server_mu):
        decay = 0.5
        self.prob_max_mu_t = decay * self.prob_max_mu_t + (1-decay) * server_mu

    def set_prob_max_var_t(self, server_var):
        decay = 0.5
        self.prob_max_var_t = decay * self.prob_max_var_t + (1-decay) * server_var
