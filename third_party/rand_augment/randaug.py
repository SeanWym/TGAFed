...
class RandAugment():
    def __init__(self, N=None, M=None):
        # Numbers = N
        # max_Magnitude = M
        #图像增强操作的列表：自动对比度调整、均衡化、旋转、反转、颜色调整等等
        self.transforms = ['autocontrast', 'equalize', 'rotate', 'solarize', 'color', 'posterize', 'invert',
        ...
        #变换后图像填充的颜色
        fillcolor = 128
        #对应增强操作的幅度范围的字典ranges，用于控制每种操作的幅度取值范围
        self.ranges = {
            # these  Magnitude   range , you  must test  it  yourself , 
            # see  what  will happen  after these  operation ,
            ...
            "autocontrast": [0] * 10,#
            "equalize": [0] * 10, #          
            "invert": [0] * 10 #上面没有  transforms里加一个
        }
        #定义图像增强函数，接收图像和上述的幅度参数ranges
        self.func = {
    ...
    #用于生成一组图像增强操作，N代表要应用的增强操作的数量，M代表增强操作的最大幅度
    ...
    def __call__(self, image, N=None, M=None):
        ...
        #找到对应的图像增强函数operation和对应的幅度大小mag  开始操作生成图像image
    ...
    def rotate_with_fill(self, img, magnitude):
        #  I  don't know why  rotate  must change to RGBA , it is  copy  from Autoaugment - pytorch  笑死我也不懂
...
if __name__ == '__main__':
    ...
    img_augment = Rand_Augment()
    img_origal = Image.open(r'F:\project\FedMatch-main\imgs\qizhixiaogou.jpg')
    for i in range(0, 10):
