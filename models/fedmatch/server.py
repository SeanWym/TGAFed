...
class Server(ServerModule):
    ...
    def _train_clients(self):
        ...
        # tatal_var = np.sum(var_values)
        # total_mask = np.sum(mask_values)
        # normalized_weights_mask = [value / total_mask for value in mask_values]  # 归一化权重
        # self.server_mu = [weight * total_mu for weight in normalized_weights_mask]
        # self.server_var = [weight * tatal_var for weight in normalized_weights_mask]
        # 传回每个客户端
        # for client_id, server_mu, server_var in zip(self.connected_ids, self.server_mu, self.server_var):
        #     if client_id in self.clients:
        #         self.clients[client_id].set_prob_max_mu_t(server_mu)
        #         self.clients[client_id].set_prob_max_var_t(server_var)
        # 计算平均值
        ...
        # 将平均值传回每个客户端
        for client_id in self.connected_ids:
            if client_id in self.clients:
                self.clients[client_id].set_prob_max_mu_t(self.server_mu)
                self.clients[client_id].set_prob_max_var_t(self.server_var)
        #########cursor添加222
        #训练完所选的那些客户端后
    ...
    def restore_clients(self, updates):
        rid = 0
        self.rid_to_cid = {}
        for cwgts, csize, cid, _, _ , mu_t, var_t ,sum_mask in updates:
